...
use crate::{Sphere, Uniforms, MAX_OBJECT_COUNT};
...
pub fn clear_all_selections(scene: &mut [Sphere; MAX_OBJECT_COUNT]) {
    for object in scene {
        object.is_selected = 0;
    }
}

pub fn add_selection(object_num: usize, scene: &mut [Sphere; MAX_OBJECT_COUNT]) {
    scene[object_num].is_selected = 1;
}

pub fn remove_selection(object_num: usize, scene: &mut [Sphere; MAX_OBJECT_COUNT]) {
