class Celestial {
	constructor(name, dayLength, yearLength, leapSeconds, initialYearProgress = 0, initialWeekDay = 0) {
		...
		this.block2YearLength =
			Math.floor(this.LeapYearFreq2 / this.LeapYearFreq1) * this.block1YearLength +
			(this.LeapYearFreq2 % this.LeapYearFreq1) * this.hYearLength_ms +
			this.hDayLength_ms;
	...
	getMonthLength(month, year) {
		...
		if (this.name === "Earth") {
			...
			if (month === 2) {
				// February
				monthLength = 28;
				if (year % this.LeapYearFreq1 === 0) monthLength++;
				if (year % this.LeapYearFreq2 === 0 && year !== 0) monthLength++;
		...
		// No month should have more than 2 days added
			...
			if (longMonthInterval < 1) console.log("The interval between longer months for " + this.name + " is invalid");

			// If the month is on the interval, it has an extra day
			if ((month - 1) % longMonthInterval === 0) monthLength++;
			...
			if (year % this.LeapYearFreq1 === 0) remainderDays++;
			if (year % this.LeapYearFreq2 === 0) remainderDays++;

			if (remainderDays > this.monthCount) console.log(this.name + " has more days left over than it has months (this should not be possible)");

			// The last few months also have days added to make up for the remainder
			if (this.monthCount - month < remainderDays) monthLength++;
		}
		return monthLength;
	}
	...
	getWeekDay(msFromYearStart, y) {
		...
		if (y % this.LeapYearFreq1 === 0) msFromEpoch -= this.hDayLength_ms;
		if (y % this.LeapYearFreq2 === 0 && !(this.name === "Earth" && y === 0)) msFromEpoch -= this.hDayLength_ms;
		...
		// Add fudge factor to account for Earth year 0 being a leap year
		if (this.name === "Earth" && msFromEpoch > this.dayLength_ms) msFromEpoch += this.dayLength_ms;
		...
		if (this.hDayOfWeek === 0) this.hDayOfWeek = 7;
	...
	checkHour(i, dayLength) {
		...
		if (curDigit < maxDigit) {
			for (let j = 0; j < maxDigit - curDigit; j++) {
				i = "0" + i;
			}
		}
		return i;
...
var Mars = new Celestial("Mars", 24.65979, 668.5991 * 24.623, 0, 0, 0);
// var Venus = new Celestial("Venus", 116.75 * 24, 5832.6, 0, 0, 0);

var Ceres = new Celestial("Ceres", 9.07417, 1683.14570801 * 24, 0, 0, 0);
...
function updateInternalValues(msFromEpoch) {
	bodies.forEach((body) => {
		body.updateDateTime(msFromEpoch);
	});
}
...
function updateFlavour(table, id, data) {
	...
	if (data[0] === "Earth")
		...
		p.innerHTML =
			data[0] + "'s " + data[5] + " day year is comprised of " + data[6] + " months, each between " + data[7] + " and " + (data[7] + 2) + " days long";
