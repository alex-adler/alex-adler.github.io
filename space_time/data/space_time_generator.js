...
class Celestial {
    ...
    setPhysicalParameters(GM_km3_S2, radius_km) {
        ...
        if (this.semiMajorAxis_0_km != undefined)
            this.period_ms = 2 * Math.PI * (this.semiMajorAxis_0_km ** 3 / this.GM_km3_s2) ** 0.5 * 1000;
    }
    ...
    setTrueAnomaly(trueAnomaly_deg, trueAnomaly_sec_Cy = 0) {
        ...
        let eccentricAnomaly_deg = Math.acos((this.eccentricity_0 + Math.cos(degToRad(this.meanAnomaly_0_deg))) / (1 + this.eccentricity_0 * Math.cos(this.meanAnomaly_0_deg)));
